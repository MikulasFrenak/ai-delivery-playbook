...
- [x] Test against a fresh Claude Code session with this repo NOT cloned — **fully verified.** `mcp-server/test.js` covers the wire protocol (stdio JSON-RPC, happy + error paths, 13 skills served). Beyond that: confirmed live from the Claude Code VS Code extension, opened in `review-spa` (a separate project, ai-delivery-playbook not open) — asked a natural-language question about design review, it called `search_skills` then `get_skill` on its own and returned an accurate summary of `skills/design-brief.md`'s actual content (trigger, prerequisites, flow, and a correct real-world caveat that review-spa itself doesn't have a Figma/token setup this skill would map onto). Connection required a project-level `.mcp.json` in `review-spa` — the CLI's `--scope user` registration does not carry over to the VS Code extension (see `setup.md`).
