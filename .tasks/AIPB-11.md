...
- [x] Test against a fresh Claude Code session with this repo NOT cloned — **fully verified, across all four client surfaces:**
  - `mcp-server/test.js` — wire protocol itself (stdio JSON-RPC, happy + error paths, 13 skills served).
  - **Claude Code CLI** — `claude mcp add --scope user`, confirmed via `claude mcp list`.
  - **Claude Code VS Code extension** — opened in `review-spa` (ai-delivery-playbook not cloned there), asked a natural-language question about design review; it called `search_skills` then `get_skill` on its own and returned an accurate summary of `skills/design-brief.md`'s actual content, including a correct real-world caveat that review-spa doesn't have the Figma/token setup that skill assumes. Required a project-level `.mcp.json` in `review-spa` — the CLI's `--scope user` registration doesn't carry over to the extension (see `setup.md`).
  - **Claude Desktop app + Cowork** — added to `claude_desktop_config.json`; this single file covers *both* the regular Desktop chat and Cowork sessions, even though Cowork runs in its own sandboxed environment (unexpected — worth knowing, since the CLI/extension split made "per-client config" seem like the universal rule, and this is the counterexample). Verified directly inside a Cowork session by calling `search_skills("postmortem incident")` and getting the real `skills/postmortem.md` description back.
