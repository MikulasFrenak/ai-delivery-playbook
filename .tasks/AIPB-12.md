...
- [x] **Live cross-network verification.** Confirmed two ways: (1) real `curl` calls against `https://ai-delivery-playbook.mikulas-frenak.workers.dev/mcp` — `initialize` returns correct `serverInfo`, `tools/call` → `search_skills("commit")` returns real skill descriptions from the deployed bundle; (2) `claude mcp add --transport http ai-delivery-playbook --scope user https://ai-delivery-playbook.mikulas-frenak.workers.dev/mcp` — the Claude Code CLI connects directly to the remote URL and works. Same confidence level as AIPB-11's stdio verification now reached, just over HTTP.
