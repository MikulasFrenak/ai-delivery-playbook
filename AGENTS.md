...
**Selector drift and flaky E2E tests are a maintenance-cost problem, not just a debugging annoyance.** If broken selectors after UI changes are a recurring source of red builds, see [`docs/test-maintenance.md`](./docs/test-maintenance.md) for the underlying architecture and vocabulary, and run [`/self-healing-selectors`](./skills/self-healing-selectors.md) to scaffold it — but note that doc's own point that selector healing addresses roughly a quarter to a third of flaky-test causes; async/timing issues and quarantine practices (also covered there) are separate, larger levers.
...
#### Worktree-Isolated Parallel Agents (real write work, not just read-only fan-out)

The "never write concurrently" rule above assumes every agent shares one working tree. A git worktree removes that assumption for genuinely independent *write* tasks: each agent gets its own isolated checkout and branch, so there's no shared file to collide over — the coordination cost moves from "don't touch the same file" to "merge each branch back once its agent is done."

- **The independence test is the same one Fan-Out uses, just for writes instead of reads:** would these two tasks touch different files even if run one after another? If yes, worktree isolation removes the wait; if a change in task A would change what task B should do, they're not independent — serialize them, a worktree doesn't fix a real dependency, it just hides the collision until merge time.
- **Brief each agent as if it knows nothing about the other's task** — state explicitly which files/directories are out of scope for it, and why (a parallel task is using them). An agent that isn't told this can wander into the other task's files "to be helpful" and reintroduce the exact conflict isolation was supposed to prevent.
- **Each agent commits on its own branch inside its worktree; it doesn't push or open a PR itself.** The orchestrating agent/human reviews each branch's actual diff before merging — worktree isolation prevents a file collision, it doesn't substitute for review of what got written.
- **A worktree agent that finds nothing to fix should say so and make no commit** — an empty branch/commit manufactured just to "have done something" is worse than an honest null result; see `examples/AIPB-19.md` for a real case of exactly this.
- **This is heavier than shell-level fan-out** (a new branch, a new checkout, a merge step) — reach for it when the *writes* themselves are independent and worth real wall-clock parallelism, not as a default way to run two agents. Most of this repo's own multi-step work is still sequential for a reason: most steps genuinely depend on the previous one's output.
