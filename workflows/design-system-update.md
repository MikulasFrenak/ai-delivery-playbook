...
1. [`create-task`](../skills/create-task.md) — usually the Feature or Chore template depending on whether it's new capability or a token/refactor change. In Step 4 (Explore the Codebase), explicitly search for every consumer of the component/token being changed — this list belongs in the task file's Files to Touch or Open Questions, not discovered mid-implementation.
